...
[services](https://github.com/FIWARE/tutorials.Relationships-Linked-Data/blob/master/services) Bash スクリプトを
...
1対多のリレーションシップを追加するには、 `object` 属性の値として配列を追加します。これは、追加データなしの単純なリンク
に使用できます。このメソッドは、**Store** に **Shelf** エンティティを `furniture` として追加するために使用されます。
